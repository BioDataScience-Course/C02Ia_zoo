...
```{r acp, record='ROP', object='zoo2_pca', arg='call,sdev'}
SciViews::R("explore")
zoo2_pca <- ___
chart$___
```
...
<!--% Pour chacune des trois méthodes (k plus proche voisin, arbre de partitionnement et forêt aléatoire) entraînez un classifieur et optimisez-le en choisissant les meilleurs paramètres pour chacun d'eux (mesure des performances par validation croisée dix fois). Indiquez les différentes valeurs des paramètres testés mais ne conservez que le meilleur modèle (ne dupliquez pas les chunks !) -->
